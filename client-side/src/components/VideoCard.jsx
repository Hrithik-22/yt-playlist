    ...
    <div className="gap-2 grid grid-cols-2 sm:grid-cols-3 p-4 bg-slate-700">
      {sortedCardsData.map((card, index) => (
        <Card key={index} radius="none" shadow="lg" className="cursor-pointer " >
          <CardHeader>
            <Image src={card.thumbnail} height={500}  alt={card.videoTitle}  radius="none"/>
          </CardHeader>
          <CardBody>
            <h3 className="font-extrabold mb-4">{card.videoTitle}</h3>
            <div className="ml-4 flex flex-col gap-y-2">
              <p className="flex items-center gap-2 ">
              <IoTimeSharp /> {card.duration.hour} hours, {card.duration.min} mins, {card.duration.secs} secs
              </p >
              <Divider />

              <p className="flex items-center gap-2 "><FaEye /> {card.viewsCount}</p>
              <Divider />

              <p className="flex items-center gap-2 "><FaRegThumbsUp /> {card.likeCount} </p>
              <Divider />

              <p className="flex items-center gap-2 "><FaRegMessage /> {card.commentCount}</p>
              <Divider />
